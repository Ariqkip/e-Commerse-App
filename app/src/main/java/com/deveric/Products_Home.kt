...
import com.deveric.Services.CustomAdapter
import com.deveric.esales.Services.ItemsViewModel
import com.deveric.esales.Services.Pop_Alert
        ...
        db.collection("Products")
            ...
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    ...
                    var itemImage=document.data["image"]
                    var itemName=document.data["name"]
                    var itemPrice=document.data["price"]
                    data_1.add(ItemsViewModel(itemImage.toString(), itemName.toString(), itemPrice.toString()))
                }
                getProductList2()
            }
        ...
        db.collection("Products")
            ...
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    ...
                    var itemName=document.data["name"]
                    var itemPrice=document.data["price"]
                    data_2.add(ItemsViewModel(itemImage.toString(), itemName.toString(), itemPrice.toString()))
                }
                loadProducts()
            }
            .addOnFailureListener { exception ->
